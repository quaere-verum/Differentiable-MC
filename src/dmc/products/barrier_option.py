...
class DownAndOutCallOption(Product):
    def __init__(
        self,
        maturity: float,
        barrier: float,
...
class DownAndOutPutOption(Product):
    def __init__(
        self,
        maturity: float,
        barrier: float,
        ...
        *,
        ...
        if maturity in observation_grid:
            ...
            self.register_buffer(
                ...
                torch.cat((
                    observation_grid,
                    torch.tensor([maturity], dtype=observation_grid.dtype, device=observation_grid.device),
                ))
            )
    ...
    def bind(self, sim_grid: SimulationGrid):
        self._indices_monitored = sim_grid.find_times_in_grid(self.get_buffer("timegrid"))
        self._index_maturity = int(
            sim_grid.find_times_in_grid(self.get_buffer("maturity").view(1)).item()
        )
    ...
    def compute_payoff(self, simulated: SimulationResult):
        ...
        intrinsic = torch.relu(self.get_buffer("strike") - final_price)
        alive = (min_price > self.get_buffer("barrier")).to(torch.float32)
        return intrinsic * alive
    ...
    def compute_smooth_payoff(self, simulated: SimulationResult):
        ...
        intrinsic = torch.nn.functional.softplus(
            self.get_buffer("strike") - final_price,
            beta=self._softplus_beta,
        )
        alive = torch.sigmoid(
            self._barrier_beta * (min_price - self.get_buffer("barrier"))
        ).to(torch.float32)
        return intrinsic * alive
